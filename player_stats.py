...
def create_table(sql_connection,table_name,position,order_by,direction):
  ...
  # sorted_player_stats = sql_cursor.execute(f"SELECT player_name,player_position,player_score FROM {table_name}")
...
test = sql_cursor.execute('SELECT password FROM user_info WHERE team_name = "jammy"')
test = test.fetchall()
print("test", test)
print("use_info", user_info)

team_name = input('Enter your team name: ')
password = input('Enter your password: ')
print(team_name,password)
team_name_check_pointer = sql_cursor.execute(f'SELECT password,team_score FROM user_info WHERE team_name = "{team_name}"')
team_name_check = team_name_check_pointer.fetchall()
print(team_name_check)
# if team_name_check == password:
#   print(f'You have logged in as {team_name}. Your current score is {}')
